...
This blog post will summarize our paper, [**HYGENE: A Diffusion-based Hypergraph Generation Method**](http://arxiv.org/abs/2408.16457), accepted at **AAAI-25**. To the best of our knowledge, this work introduces the first deep-learning based method for hypergraph generation. Code is available [here](https://github.com/DorianGailhard/SODA_Hypergraph-generation).
...
- [3D meshes](https://en.wikipedia.org/wiki/Polygon_mesh) can be represented with triangles as 3-edges
...
- [Molecules](https://en.wikipedia.org/wiki/Molecule) can be depicted more accurately, with functional groups like carboxylic acids joined into hyperedges to highlight their higher-order relationship

<style>
.flex-equal-height-row {
  display: flex;
...
<div class="flex-equal-height-row">
  <!-- Left column: Electronic Circuit -->
  <div class="flex-img-col">
    {% include figure.liquid loading="eager" path="assets/img/hygene/electronic_circuit.png" class="rounded z-depth-1" zoomable=true %}
  </div>

  <!-- Right column: Molecule and Mesh stacked vertically -->
  <div class="flex-img-col stacked-images">
    {% include figure.liquid loading="eager" path="assets/img/hygene/molecule.png" class="rounded z-depth-1 mb-2" zoomable=true %}
    {% include figure.liquid loading="eager" path="assets/img/hygene/mesh.png" class="rounded z-depth-1" zoomable=true %}
  </div>
</div>
...
Our work builds upon the foundation laid by the paper [Efficient and Scalable Graph Generation through Iterative Local Expansion](https://arxiv.org/abs/2312.11529), which introduces an innovative method for graph generation. This approach draws inspiration from the tradition of graph coarsening algorithms.
...
<div class="caption">
    At each step, a first model chooses which nodes need to be duplicated before a second model filters out the resulting edges.
</div>
